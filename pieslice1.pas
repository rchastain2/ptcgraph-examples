...
begin
  ...
  _To := _From + Round(percent * 360.0);
    { percent * 360.0  -это процент от круга (360 градусов) }
  PieSlice(x, y, _From, _To, Radius);
  _From := _To;
    { Как только нарисовали 1 сектор, запоминаем конечный угол предыдущего в начало следующего }
end;
